...
Loads the database URL from application settings and the metadata from the
ORM models so that `alembic revision --autogenerate` stays in sync.
"""
...
# Escape '%' characters so passwords with percent signs survive interpolation.
